...
for i = 1:size(traffic,2)
   ...
   switch traffic(i).type
    case 'arrival'
        ...
        lambda = FirstFit(path,bwmat,bw);
        active_conn(ID).lambda = lambda;
        
        if(active_conn(ID).lambda>0)
            [bwmat, err] = BandwidthAllocate( bwmat, path, lambda, bw, ID );
            if err==0
    ...
    case 'departure'
        ...
        if(active_conn(ID).isactive == true)
            [bwmat, err] = BandwidthDeallocate( bwmat, active_conn(ID).path, active_conn(ID).lambda,bw, ID );
            if err==0
                active_conn(ID).isactive = false;
            else
